...
public class Tile
{
    ...
    public bool IntersectRay(Ray ray, Transform transform)
    {
        Matrix4x4 trs = Matrix4x4.TRS(transform.position, transform.rotation, transform.localScale);

        var avgPosition = trs.MultiplyPoint3x4(this.averagePosition);
        var norm = (transform.rotation * this.normal).normalized;
        var r = this.radius * Math.Max(transform.localScale.x, Math.Max(transform.localScale.y, transform.localScale.z));

        if (!Utils.IntersectRayWithSphere(ray, avgPosition/*this.averagePosition*/, r /*this.radius*/)) return false;

        if (Utils.SignedDistancePlanePoint(norm /*this.normal*/, avgPosition/*this.averagePosition*/, ray.origin) <= 0) return false;

        var denominator = Vector3.Dot(norm /*this.normal*/, ray.direction);
        if (denominator == 0) return false;


        var t = -(Vector3.Dot(ray.origin, norm /*this.normal*/) + Utils.PlaneConstant(norm /*this.normal*/, avgPosition /*this.averagePosition*/)) / denominator;
        var point = (ray.direction * t) + ray.origin;
        var origin = Vector3.zero;
        for(var i = 0; i < this.corners.Length; ++i)
        {
            var j = (i + 1) % this.corners.Length;
            var sideNormal = transform.rotation * Vector3.Cross(this.corners[j].position, this.corners[i].position);
            var sideOrigin = transform.position; //origin;
